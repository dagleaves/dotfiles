...
Main-session policy. If you are running as a subagent role (scout, Explore, mech-executor, executor, security-executor), ignore this section entirely and just do the task you were given.
...
- Front-load context into every delegation prompt: exact paths, relevant excerpts, the diff under review - anything you already know. A sub-agent that has to rediscover it costs 10-100x the tokens of pasting it.
- A single grep/read you can do in one call is cheaper than any scout - the cheapest agent still costs ~100k+ lifetime tokens once it starts exploring.
