...
Context economy: the spec names the relevant paths - start there instead of re-exploring the repo. Read targeted sections of large files rather than whole files, and don't re-read files already in your context. Thoroughness here means covering the trust boundaries the task touches, not re-reading the whole codebase.
